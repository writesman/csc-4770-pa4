  ...
  while (true) {
    ...
    if (req.command == Protocol::CMD_LOCK) {
      manager->acquire(req.resource,
                       req.mode); // This call blocks until the lock is secured.
    ...
    // Send reply back to router: [Client ID | Empty | Reply]
...
std::string SessionRegistry::get_worker_for_client(const std::string &client_id,
  ...
  // Check for worker affinity (Client ID -> Worker ID).
  ...
  // Wait for the new worker's MSG_READY signal on the backend socket
  // (synchronization).
